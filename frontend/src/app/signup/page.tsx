  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/popup-blocked') {
        setError('Popup blocked! Please check your address bar (usually on the right) and select "Allow Popups" for this site.');
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
          <div className="glass rounded-[3rem] p-10 shadow-2xl border border-glass-border relative overflow-hidden">
            ...
            <form onSubmit={handleSignup} className="flex flex-col gap-4">
               ...
               <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} disabled={loading} className="w-full bg-foreground text-background p-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-orange-600 hover:text-white shadow-xl transition-all cursor-pointer premium-button">
            ...
            <button onClick={handleGoogleSignup} className="w-full flex items-center justify-center gap-4 bg-background dark:bg-stone-900/50 border border-glass-border p-5 rounded-2xl font-black uppercase tracking-widest text-[10px] text-foreground hover:bg-foreground/5 transition-all cursor-pointer active:scale-[0.98] premium-button">
            ...
            <div className="text-center mt-8">
              <Link href="/login" onClick={() => playSound('click')} className="text-xs font-black uppercase tracking-widest text-stone-400 hover:text-orange-600 transition-colors">Already have an account? Login</Link>
            </div>
          </div>
        </motion.div>
      </main>
