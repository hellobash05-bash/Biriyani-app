  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (err.code === 'auth/popup-blocked') {
        setError('Popup blocked! Please click the icon in your address bar (usually on the right) to "Allow Popups" for this site.');
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full max-w-md relative z-10">
        <div className="premium-card !bg-background/80 backdrop-blur-xl border border-glass-border p-10 rounded-[3rem] shadow-2xl">
          ...
          <button 
            ...
            disabled={loading} 
            className="w-full flex items-center justify-center gap-4 bg-foreground/5 border border-glass-border p-5 rounded-2xl font-black uppercase tracking-widest text-[10px] text-stone-500 hover:bg-foreground/10 transition-all cursor-pointer mb-6 active:scale-[0.98]"
          >
