  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err.code === 'auth/popup-blocked') {
        setError('Popup blocked! Please click the icon in your address bar (usually on the right) and select "Allow Popups" for this site.');
          ...
          <div className="premium-card !bg-background/95 rounded-[3.5rem] p-12 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.8)] border border-glass-border relative overflow-hidden backdrop-blur-3xl">
            ...
            <div className="flex flex-col gap-4">
               <button 
                 ...
                 className="w-full flex items-center justify-center gap-4 bg-transparent border border-glass-border p-6 rounded-2xl font-black uppercase tracking-[0.15em] text-xs text-foreground/80 hover:border-orange-500 transition-all cursor-pointer disabled:opacity-50 active:scale-[0.98] premium-button"
               >
               ...
               <form onSubmit={handleEmailAuth} className="flex flex-col gap-4">
                  ...
                  <motion.button whileHover={{ scale: 1.01, backgroundColor: '#ea580c' }} whileTap={{ scale: 0.99 }} disabled={loading} className="w-full bg-orange-600 text-white p-6 rounded-2xl font-black uppercase tracking-widest text-sm shadow-[0_20px_40px_-15px_rgba(245,158,11,0.3)] transition-all cursor-pointer mt-2 premium-button">
                    {loading ? 'Logging in...' : 'Login with Email'}
                  </motion.button>
               </form>

               <div className="text-center mt-12 flex flex-col gap-4 relative z-50">
                   <Link href="/signup" onClick={() => playSound('click')} className="text-xs font-black uppercase tracking-widest text-stone-400 hover:text-orange-600 transition-colors cursor-pointer">
                   ...
                   <div className="mt-4 pt-6 border-t border-glass-border">
                     ...
                     <div className="flex flex-col gap-3">
                       <button 
                         ...
                         className="bg-orange-500/10 text-orange-500 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest border border-orange-500/20 premium-button"
                       >
